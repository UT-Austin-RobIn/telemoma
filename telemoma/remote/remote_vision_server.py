...
class RemoteVisionTeleopServer(BaseTeleopInterface):
    
    def __init__(self, set_ref=True, *args, **kwargs):
    ...
    def start(self) -> None:
        ...
        print("\033[93m" + "Waiting for client connection..." + "\033[0m")
        server_socket, addr = server_socket.accept()
        self.server_socket = SocketConnection(server_socket)
    ...
    def _update_internal_state(self, hz=50):
        while True:
            data = self.server_socket.read_data()
            self.latest_pose = {'body': data['data']}
...
if __name__=='__main__':
    ...
    detector = RemoteVisionTeleopServer(set_ref=True)
    detector.start()

    while 1:
        print(detector.get_hip_position_and_rotation())
        time.sleep(0.1)
